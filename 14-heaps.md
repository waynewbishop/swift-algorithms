...
```swift
public struct DaryHeap<Element: Comparable> {
    private var elements: [Element] = []
    private let branchingFactor: Int
    private let priorityFunction: (Element, Element) -> Bool

    public init(branchingFactor: Int = 4, type: HeapType) {
        self.branchingFactor = max(2, branchingFactor)
        switch type {
        case .minHeap:
            priorityFunction = { $0 < $1 }
        case .maxHeap:
    ...
    private func childIndices(of index: Int) -> [Int] {
        let firstChild = branchingFactor * index + 1
        return Array(firstChild..<min(firstChild + branchingFactor, elements.count))
    }
}
```
...
extension Heap {
    public func visualize() -> String {
        guard !isEmpty else { return "Empty heap" }
        ...
        while index < elements.count {
            let spacing = String(repeating: "  ", count: (1 << (3 - level)) - 1)

            for _ in 0..<elementsInLevel {
                if index < elements.count {
                    result += spacing + "\(elements[index])" + spacing
                    index += 1
                }
            }
...
Heaps demonstrate several important algorithmic concepts:

1. **Complete Binary Trees**: Efficient array representation
2. **Invariant Maintenance**: Preserving heap property through operations
3. **Amortized Analysis**: Insert/extract operations average out over time
4. **Priority-Based Processing**: Natural fit for priority queues
